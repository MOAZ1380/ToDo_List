...
// @desc    VerifyCode
// @route   GET /api/auth/VerifySignup
// @access  Public
exports.VerifySignup = asyncHandler(async (req, res, next) => {
	...
	if (user.otpExpire < new Date()) {
		return next(
			new ApiError(400, "Verification code has expired. Please sign up again."),
		);
	}
	...
	return res
		.status(200)
		.json({ message: "Account verification has been successful", user, token });
});

// @desc    VerifyCode
// @route   GET /api/auth/VerifyPassword
// @access  Public
...
exports.ForgetPassword = asyncHandler(async (req, res, next) => {
	...
	res
		.status(200)
		.json({ message: "Please check your email for verification code" });
});
